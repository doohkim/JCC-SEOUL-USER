...
function formatFamilyRelationship(f) {
  if (!f || typeof f !== "object") return "";
  if (f.relationship === "other" && f.relationship_note) {
    return String(f.relationship_note);
  }
  const code = f.relationship;
  return FAMILY_REL_LABEL[code] || String(code || "");
}

/** 소속: 직접 입력 텍스트 + 연결된 부서명(있으면) */
function formatFamilyAffiliation(f) {
  if (!f || typeof f !== "object") return "";
  const text = (f.affiliation_text || "").trim();
  const divName = (f.division_name || "").trim();
  if (text && divName) return `${text} · ${divName}`;
  return text || divName;
}

function formatFamilyRemarks(f) {
  if (!f || typeof f !== "object") return "";
  return (f.remarks || "").trim();
}

function renderFamilyDetailTable(family) {
  const tb = document.getElementById("tbodyFamilyDetail");
  if (!tb) return;
  tb.innerHTML = "";
  const rows = Array.isArray(family) ? family : [];
  if (!rows.length) {
    tb.innerHTML = `<tr><td colspan="5">등록된 가족 정보가 없습니다.</td></tr>`;
    return;
  }
  rows.forEach((f) => {
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHtml(formatFamilyRelationship(f))}</td>
      <td>${escapeHtml(formatFamilyAffiliation(f))}</td>
      <td>${escapeHtml(f.church_position || "")}</td>
      <td>${escapeHtml(formatFamilyRemarks(f))}</td>
    `;
    tb.appendChild(tr);
  });
}
...
async function loadDetail() {
  const memberId = parseMemberId(window.__MEMBER_ID__);
  if (memberId == null) throw new Error("member id missing");
  setStatus("불러오는 중…");
  ...
  let familyList = [];
  try {
    const famRaw = await apiGet(`/member/${memberId}/family/`);
    familyList = Array.isArray(famRaw) ? famRaw : [];
  } catch (e) {
    console.warn("가족 목록 API:", e);
    familyList = Array.isArray(data.family) ? data.family : [];
  }
  renderFamilyDetailTable(familyList);

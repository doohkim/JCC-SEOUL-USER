"""관리자: 사용자(User)·교적(Member) 부서·팀 이동."""
...
from users.forms.member_org_move import (
...
def member_org_move_dashboard(request):
    return render(
        request,
        "admin/users/member_org_move_dashboard.html",
        {
            "title": "교적 — 부서·팀 이동",
            "opts": Member._meta,
        },
...
def member_org_move_detail(request, object_id):
    ...
    return render(
        request,
        "admin/users/member_org_move_detail.html",
        {
            ...
            "back_url": reverse("admin:users_member_change", args=[member.pk]),
            "opts": Member._meta,
        },
    )

...
class CounselingHomeView(OnboardingRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["can_manage_counseling"] = can_access_counseling_manage_tab(
            self.request.user
        )
        return ctx


class CounselingRequestDetailView(
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        counseling_request_detail_for_user(
            user=request.user, public_id=kwargs["public_id"]
        )
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_context_data(self, **kwargs):
        ...
        req = counseling_request_detail_for_user(
            user=self.request.user, public_id=self.kwargs["public_id"]
        )

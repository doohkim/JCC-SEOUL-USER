...
- **할 일 목록**: TodoWrite 항목 설명은 **한글**로 작성한다 (`.cursor/rules/01-jcc-seoul-django.mdc`와 동일).
- **권한**: `users/permissions.py` — 출석·부서 목록은 `visible_divisions_for`(소속 Division만). 교적은 `can_access_member_registry`(목사·전도사). **수련회**는 `retreat/services/staff_capabilities.py`의 `effective_capabilities` + `users/permissions.py` 래퍼가 단일 진실. API/Admin 추가 시 반드시 이 계층을 따른다.

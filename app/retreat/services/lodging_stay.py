...
def resolve_lodging_stay_status(attendee: RetreatAttendee) -> str:
    ...
    if effective_status(attendee) == RetreatAttendee.CheckInStatus.CHECKED_OUT:
        return S.ENDED
...
def lodging_stay_display(attendee: RetreatAttendee) -> str:
    """목록·API용 숙소 칸 표시 문자열."""
    S = RetreatAttendee.LodgingStayStatus
    status = resolve_lodging_stay_status(attendee)
    if status == S.ACTIVE:
...
def active_lodging_occupant_q(*, prefix: str = "") -> Q:
    """QuerySet filter: 현재 시각 기준 활성 숙박자."""
    ...
    return (
        Q(
            **{
                f"{p}participation_status": RetreatAttendee.ParticipationStatus.PARTICIPATING
            }
        )
        & ~effective_status_q(
        ...
        & Q(
            **{
                ...
                f"{p}lodging_room__isnull": False,
            }
        )
    )
...
def active_lodging_occupant_filter(qs: QuerySet) -> QuerySet:
    return qs.filter(active_lodging_occupant_q())
...
def lodging_stay_eligible_filter(qs: QuerySet) -> QuerySet:
    ...
    computed = (
        Q(participation_status=P.PARTICIPATING)
        & ~effective_status_q(C.CHECKED_OUT)
